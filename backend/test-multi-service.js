    ...
    try {
      ...
      if (passed && scenario.expectedValid) {
        ...
        if (durationMatch) {
          ...
          console.log(`${colors.yellow}⚠ PARTIAL:${colors.reset} ${scenario.name}`);
          console.log(`  Expected duration: ${scenario.expectedDuration}, Got: ${data.totalDuration}`);
        }
        ...
        console.log(`  Expected valid: ${scenario.expectedValid}, Got: ${data.valid}`);
      }
...
async function testServicesCatalog() {
  ...
  try {
    ...
    if (data.services && Array.isArray(data.services)) {
      console.log(`${colors.green}✓ PASS:${colors.reset} Services catalog retrieved`);
      console.log(`  Total services: ${data.services.length}`);
      console.log(`  Shop hours: ${data.shopHours.open} - ${data.shopHours.close}`);
      
      console.log(`\n  ${colors.blue}Available Services:${colors.reset}`);
      data.services.forEach(service => {
        console.log(`    • ${service.name} (${service.duration}h) - ${service.category}`);
      });
    } else {
...
async function testAvailableSlots() {
  ...
  for (const testCase of testCases) {
    try {
      const response = await axios.get(`${BASE_URL}/available-slots/${dateString}`, {
      ...
      if (data.availableSlots && Array.isArray(data.availableSlots)) {
        ...
        if (data.totalDuration) {
          console.log(`  Duration: ${data.totalDuration} hours`);
          if (data.availableSlots.length > 0 && data.availableSlots[0].endTime) {
            console.log(`  Sample slot: ${data.availableSlots[0].startTime} - ${data.availableSlots[0].endTime}`);
          }
        }
      } else {
