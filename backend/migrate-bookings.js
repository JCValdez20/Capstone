...
// MongoDB connection
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://localhost:27017/washup";
...
function getServiceDuration(serviceName) {
  ...
  console.warn(
    `${colors.yellow}⚠ Unknown service: ${serviceName}, defaulting to 1 hour${colors.reset}`
  );
...
async function migrateBookings() {
  try {
    ...
    console.log(
      `${colors.blue}Found ${bookingsToMigrate.length} bookings to migrate${colors.reset}\n`
    );
    ...
    for (const booking of bookingsToMigrate) {
      try {
        ...
        if (!serviceName) {
          console.log(
            `${colors.yellow}⚠ Skipping booking ${booking._id}: No service name${colors.reset}`
          );
          errorCount++;
          continue;
        }
        ...
        if (endTime) {
          ...
          console.log(
            `${colors.yellow}⚠ Could not calculate end time for booking ${booking._id}${colors.reset}`
          );
        }
        ...
        successCount++;
        console.log(
          `${colors.green}✓${colors.reset} Migrated: ${
            booking._id
          } | ${serviceName} (${duration}h) | ${booking.timeSlot}${
            endTime ? ` - ${endTime}` : ""
          }`
        );
      } catch (error) {
        ...
        console.log(
          `${colors.red}✗${colors.reset} Error migrating ${booking._id}: ${error.message}`
        );
      }
    }
    ...
    await mongoose.disconnect();
    console.log(
      `${colors.green}✓ Migration complete and database disconnected${colors.reset}`
    );
  } catch (error) {
...
async function verifyMigration() {
  try {
    console.log(
      `${colors.cyan}Connecting to MongoDB for verification...${colors.reset}`
    );
    await mongoose.connect(MONGODB_URI);
    ...
    console.log(
      `  ${colors.yellow}⚠ Need migration: ${withoutServices}${colors.reset}`
    );
