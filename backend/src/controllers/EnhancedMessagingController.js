...
class MessagingController {
  ...
  async getOrCreateConversation(req, res) {
    try {
      ...
      const conversation = await MessagingService.createOrGetBookingConversation(
        bookingId,
        userId,
        userRole
      );
  ...
  async getOrCreateDirectConversation(req, res) {
    try {
      ...
      const targetUser = await MessagingService.verifyDirectMessagingUser(targetUserId);
      if (!targetUser) {
        return send.sendErrorMessage(
          res,
          404,
